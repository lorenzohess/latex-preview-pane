...
(defun lpp/line-errors ()
  (let ((old-buff (current-buffer)))
    ...
    (let ((error-msg (buffer-substring (point-min) (point-max))))
      ...
      (mapcar
       (lambda (what) (lpp/chomp (substring what 2)))
       (latex-pp-filter
        (lambda (what)
          (eq (string-match "l\\.[0-9]*" what) 0))
...
(defun lpp/line-errors-to-layovers (errors)
  (mapcar
   (lambda (what)
     (let ((line (string-to-number what)))
       (let (layoverStart
             layoverEnd)
         (goto-char (point-min))
         (forward-line (1- line))
...
(defun lpp/invoke-pdf-latex-command ()
  (let ((buff (expand-file-name (lpp/buffer-file-name)))
        (default-directory
         (file-name-directory (expand-file-name (lpp/buffer-file-name)))))
    (if shell-escape-mode
        (call-process pdf-latex-command
                      nil
                      "*pdflatex-buffer*"
                      nil
                      "-shell-escape"
